...
/* MG996R Pulse widths (50Hz)
 * 13-bit Resolution: 2^13 = 8192
 * 20ms period (50Hz) -> 8192 ticks
 * 0.5ms (Unlock) -> (0.5/20) * 8192 = 204.8 ~= 205
 * 2.5ms (Lock)   -> (2.5/20) * 8192 = 1024
 */
#define SERVO_PULSE_LOCK     1024
#define SERVO_PULSE_UNLOCK   205
...
static void set_servo_state(bool lock)
{
    uint32_t duty = lock ? SERVO_PULSE_LOCK : SERVO_PULSE_UNLOCK;
    ledc_set_duty(SERVO_LEDC_MODE, SERVO_LEDC_CHANNEL, duty);
    ledc_update_duty(SERVO_LEDC_MODE, SERVO_LEDC_CHANNEL);
    ESP_LOGI("app_driver", "Servo set to %s (Duty: %"PRIu32")", lock ? "Lock" : "Unlock", duty);
}
...
{
    set_servo_state(target);
    app_indicator_set(target);
    /* Trigger integrated status update and report to cloud */
